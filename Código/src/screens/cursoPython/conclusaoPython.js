...
export default function TelaConclusaoPython() {
    ...
    useEffect(() => {
        async function carregarDados() {
            ...
            const { data: conquistaDesbloqueada, error: conquistaDesbloqueadaError } = await supabase
                .from('conquistas_desbloqueadas')
                .select('id, idusuario, conquista')
                .eq('idusuario', user.id)
            if (conquistaDesbloqueadaError) {
                console.error('Erro ao buscar conquista desbloqueada:', conquistaDesbloqueadaError.message);
                setLoading(false);
                return;
            }
            ...
            if (conquistaInfo) {
                setItemNome(conquistaInfo[0].nome);
                setItemImagem(conquistaInfo[0].imagem);
            }
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            <View style={styles.container}>
                ...
                <Text style={styles.subtitulo}>Você concluiu o curso de Python!</Text>
                ...
                <Text style={styles.texto}>Você desbloqueou: <Text style={{ fontWeight: 600 }}>{itemNome}</Text> </Text>
